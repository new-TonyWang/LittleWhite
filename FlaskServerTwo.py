...
flags.DEFINE_string('image', './test_image_dir/1571060467010.jpg',
                    'Image path to run hed, must be jpg image.')
flags.DEFINE_string('checkpoint_dir', './checkpoint',
                    'Checkpoint directory.')
flags.DEFINE_string('image_dir', 'C:\\Users\\Owner\\Desktop\\keyan\\hedForQr\\hedForQr\\image_dir_for_android',
                    'Checkpoint directory.')
flags.DEFINE_string('output_dir', './test_image_result',
                    'Output directory.')
FLAGS = flags.FLAGS
...
def upload():
    if request.method == 'POST':
        ...
        upload_path = "C:\\Users\\Owner\\Desktop\\keyan\\hedForQr\\hedForQr\\image_dir_for_android"+"\\this.jpg"
        # 保存文件
        f.save(upload_path)
        # 返回上传成功界面
        if not os.listdir(FLAGS.output_dir):
            print("111")
            time.sleep(0.1)
        #dsn_fuse_image_path = os.path.join(FLAGS.output_dir, "thistime.jpg")
        dsn_fuse_image_path = os.path.join(
            "C:\\Users\\Owner\\Desktop\\okhttpFlaskDemo-master\\okhttpFlaskDemo-master\\FlaskServer\\test_image_result\\",
            "thistime.jpg")

        print(dsn_fuse_image_path)
        src = cv.imread(dsn_fuse_image_path)
        changdu = line_detect_possible(src, dsn_fuse_image_path)
        os.remove(dsn_fuse_image_path)
