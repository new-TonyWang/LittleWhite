...
public final class FECParameters {
    ...
    public static FECParameters newParameters(long dataLen, int symbSize, int numSrcBs, int interLen) {
        ...
        final int Al = ParameterChecker.symbolAlignmentValue();//閺堝妫舵０锟�
    ...
    public static Parsed<FECParameters> ExtractFECParameters(byte[] array) {

        return parse(ByteBuffer.wrap(array, 0, 12));
    }
    /**
    ...
    public static Parsed<FECParameters> parse(ByteBuffer buffer) {
        ...
        if (buffer.remaining() < required) {
            ...
            final long commonFecOTI = buffer.getLong();   // 8 bytes閿涘苯鎮滄稉瀣鐠囷拷8byte閵嗗倶锟藉倻鍔ч崥锟�8*8=64閸掓艾銈芥稉绨俹ng閻ㄥ嫬顔愰柌蹇ョ礉閸戠鍤�
    ...
    public byte[] asArray() {
        ...
        final byte[] array = new byte[SizeOf.LONG + SizeOf.INT];//8+4==12,闂�鍨娑擄拷鐎癸拷
