...
public  class RGBGridSampler extends GridSampler {
    @Override
    ...
    public BitMatrix[] RGBGrid(RGBData image, int dimensionX, int dimensionY, PerspectiveTransform transform) throws NotFoundException {
        ...
        for (int y = 0; y < dimensionY; y++) {
            ...
            checkAndNudgePoints(image.getBitMatrixB(), points);
            try {
                for (int x = 0; x < max; x += 2) {
                    //int xx = Math.round();
                    //int yy = Math.round();
                    boolean R = image.getBitMatrixR().get((int)points[x],(int)points[x+1]);
                    boolean G = image.getBitMatrixG().get((int)points[x],(int)points[x+1]);
                    boolean B = image.getBitMatrixB().get((int)points[x],(int)points[x+1]);
                  if(R&&G&&B){//黑
                      bitsR.set(x/2,y);
                      bitsG.set(x/2,y);
                      bitsB.set(x/2,y);
