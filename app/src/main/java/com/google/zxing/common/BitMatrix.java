 ...
 *
 *表示位的二维矩阵。在下面的函数参数中，以及整个公共模块中，x是列位置，y是行位置。
 *顺序总是x y，原点在左上角。在内部，bits表示为32位int的一维数组。
 *但是，每一行都以一个新的int开头，这样做是有意的，这样我们就可以非常有效地将一行复制到一个bit数组中。
 *bits的顺序是行为主的。在每个int中，首先使用最不重要的bits，这意味着它们表示较低的x值。
 *这与BitArray的实现是兼容的。
 * @author Sean Owen
...
/**
  ...
  public BitMatrix(int width, int height) {
    ...
    this.width = width;
    this.height = height;
    this.rowSize = (width + 31) / 32;//保证rowSize>=1,因为是按行存储，所以剩下的还没看明白。。。
