...
public class ColorDetector  {
    private final HsvData image;
    private ResultPointCallback resultPointCallback;
    public ColorDetector(HsvData image) {
        this.image = image;
    }
    protected final HsvData getImage() {
        return image;
    }
    ...
    public final DetectorResult[] detect(Map<DecodeHintType,?> hints, HSVColorTable hsvColorTable) throws NotFoundException, FormatException {
        ...
        FinderPatternFinder finder = new FinderPatternFinder(image.getBitMatrix(), resultPointCallback);//找到定位图标!!!!重点来了
	...
	*/
    ...
    private float sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY, int toX, int toY) {
        ...
        if (otherToX < 0) {
            ...
            otherToX = 0;
        } else if (otherToX >= image.getBitMatrix().getWidth()) {
            scale = (image.getBitMatrix().getWidth() - 1 - fromX) / (float) (otherToX - fromX);
            otherToX = image.getBitMatrix().getWidth() - 1;
        }
        ...
        if (otherToY < 0) {
            ...
            otherToY = 0;
        } else if (otherToY >= image.getBitMatrix().getHeight()) {
            scale = (image.getBitMatrix().getHeight() - 1 - fromY) / (float) (otherToY - fromY);
            otherToY = image.getBitMatrix().getHeight() - 1;
        }
    ...
    private float sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX, int toY) {
        ...
        for (int x = fromX, y = fromY; x != xLimit; x += xstep) {
            ...
            if ((state == 1) == image.getBitMatrix().get(realX, realY)) {
                if (state == 2) {
    ...
    protected final AlignmentPattern findAlignmentInRegion(float overallEstModuleSize,
        ...
        int alignmentAreaRightX = Math.min(image.getBitMatrix().getWidth() - 1, estAlignmentX + allowance);
        if (alignmentAreaRightX - alignmentAreaLeftX < overallEstModuleSize * 3) {
            throw NotFoundException.getNotFoundInstance();
        }

        int alignmentAreaTopY = Math.max(0, estAlignmentY - allowance);
        int alignmentAreaBottomY = Math.min(image.getBitMatrix().getHeight() - 1, estAlignmentY + allowance);
