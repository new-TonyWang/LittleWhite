...
final class DecodedBitStreamParser {
	 ...
	 * @throws NotDataException 
	 */
	static DecoderResult decodepayload(byte[] bytes, Version version, ErrorCorrectionLevel ecLevel,
			Map<DecodeHintType, ?> hints) throws FormatException {
		...
		case ECI:
			...
			count = bits.readBits(mode.getCharacterCountBits(version));
			CharacterSetECI	currentCharacterSetECI = CharacterSetECI.getCharacterSetECIByValue(value);
			if (currentCharacterSetECI == null) {
				throw FormatException.getFormatInstance();
			}
			StringBuilder result1 = new StringBuilder(50);
			decodeByteSegment(bits, result1, count, currentCharacterSetECI, byteSegments, hints);//hints正常为空,currentCharacterSetECI
			return new DecoderResult(bytes, result1.toString(), byteSegments.isEmpty() ? null : byteSegments,
					ecLevel == null ? null : ecLevel.toString(), -1, -1,Mode.BYTE);
			default:
				throw FormatException.getFormatInstance();
		}
	 ...
	 * @throws FormatException
	 */
	private static void decodeByte(BitSource bits,  int count, Collection<byte[]> byteSegments, Map<DecodeHintType, ?> hints)
			throws FormatException {
// Don't crash trying to read more bits than we have available.
		if (8 * count > bits.available()) {
			throw FormatException.getFormatInstance();
