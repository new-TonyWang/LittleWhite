...
flags.DEFINE_string('image_dir', './image_dir_for_android/',
                    'Checkpoint directory.')
...
if __name__ == "__main__":
        ...
        with tf.Session() as sess:
            ...
            if latest_ck_file:
                ...
                exit()
            while(True):
                filelist=os.listdir(FLAGS.image_dir)#该文件夹下所有的文件（包括文件夹）
                if not os.listdir(FLAGS.image_dir):
                    continue;
                for files in filelist:#遍历所有文件
                    Olddir=os.path.join(FLAGS.image_dir,files);#原来的文件路径
                    feed_dict_to_use = {image_path_placeholder:Olddir,
                    ...
                    line_detect_possible(src,FLAGS.image)
                    os.remove(Olddir)
